<div class="col-xl-12">
	<div class="card">
		<div class="card-header">
			<h5>Progres bimbingan skripsi ku</h5>
		</div>
		<div class="card-block">
			<div class="row">
				...
				<?php $bimbinganku = $this->db->select('*')->where('sender', $this->session->userdata('username'))->from('tb_guidance')->get()->result();?>
				<?php if(count($bimbinganku) != 0):?>
					<div class="col-sm-12 col-xl-4 sub-title text-primary">
						<div class="media">
					...
					<div class="col-sm-12 col-xl-3 sub-title">
						<?php $totalBimbingan = $this->db->select('*')->where('sender', $this->session->userdata('username'))->from('tb_guidance')->get()->result();?>
					...
					<div class="col-sm-12 col-xl-3 sub-title">
						<?php $totalFeedback = $this->db->select('*')->where('receiver', $this->session->userdata('username'))->from('tb_guidancecard')->get()->result();?>
					...
					<div class="col-sm-12 col-xl-2 sub-title">
						<a class="btn btn-mini btn-outline-primary" id="Modal-Tourist" data-toggle="modal" data-target="#modal_see_detailku<?= $this->session->userdata('username');?>" class="btn btn-mini btn-outline-success"><i class="ti-eye"></i>Detail</a>
					</div>
					<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="modal_see_detailku<?=$this->session->userdata('username');?>" aria-hidden="true">
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title" id="exampleModalLabel">Data bimbingan <?= $this->session->userdata('name');?></h5>
