<div class="col-xl-12">
	...
	<a href="<?= site_url('TU/dashboard/daftar-pendadaran-mahasiswa');?>" class=""><i class="ti-back-left"></i> Kembali</a>
	<div class="card">
		<div class="card-header" style="background-color: #75A8FE;">
			<h5 style="color: white;">Detail data dokumen pengajuan pendadaran mahasiswa</h5>
		</div>
		<div class="card-block">
			<div class="row">
				...
				<?php foreach($DataMhs as $mhs):?>
					...
					<div class="col-sm-12 col-xl-6 sub-title text-right">
						<?php $thsrch = $this->db->select('*')->where('nim', $mhs->username)->from('tb_thesisreceived')->get()->result();?>
						<?php if(count($thsrch) !=0 ):?>
							<?php foreach($thsrch as $srch):?>
								<?php if ($srch->status_daftar == 1):?>
								<a onclick="fungsiDaftarkan(<?= $mhs->username;?>)" class="btn btn-mini btn-outline-primary"><i class="ti-check"></i>Daftarkan pendadaran <?= $mhs->fullname;?></a>
								<?php else:?>
									<a class="btn btn-mini btn-outline-primary disabled"><i class="ti-check"></i><?= $mhs->fullname;?> Sudah terdaftar</a>
								<?php endif;?>
							<?php endforeach;?>
						<?php else:?>
						<?php endif;?>
					...
					<?php foreach($DataDokumen->result() as $dock):?>
						...
						<div class="col-sm-12 col-xl-3 sub-title text-center text-primary">
							<?php if($dock->status == 0):?>
								<a onclick="fungsiApp(<?= $dock->idoc;?>)" class="btn btn-mini btn-outline-primary"><i class="ti-check"></i>Approved</a>
								<a href="" class="btn btn-mini btn-outline-danger" id="Modal-Tourist" data-toggle="modal" data-target="#modal_revisi<?= $dock->idoc;?>"><i class="ti-marker-alt"></i>Revisi</a>
							<?php else:?>
						...
						<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="modal_seea<?= $dock->idoc;?>" aria-hidden="true">
							<div class="modal-dialog modal-lg">
								<div class="modal-content">
									...
									<object data="<?=base_url('_uploads/pendadaran/'.$dock->fullname.'/'.$dock->file);?>" width="100%" height="500"></object>
								</div>
							</div>
						</div>

						<!-- modal revisi document -->
						<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="modal_revisi<?= $dock->idoc;?>" aria-hidden="true">
							<div class="modal-dialog modal-lg">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="exampleModalLabel">Revisi document syarat pendadaran</h5>
										<button type="button" class="close" data-dismiss="modal" aria-label="Close">
											<span aria-hidden="true">&times;</span>
										</button>
									</div>
									<form action="<?= site_url('TU/dashboard/kirim-revisi-dokumen');?>" method="post">
...
<script type="text/javascript">
	function fungsiApp(id) {
		$.ajax({
			type: "POST",
			url: '<?= site_url("TU/dashboard/approved-dokumen")?>',
			data: {
				id: id
			},
			error: function(data) {
	...
	function fungsiDaftarkan(id) {
		$.ajax({
			type: "POST",
			url: '<?= site_url("TU/dashboard/daftarkan-mahasiswa-ini")?>',
			data: {
				id: id
			},
			error: function(data) {
